...
class LarkCliManager:
    ...
    def ensure_cli(self) -> tuple[str, bool]:
        ...
        # 通过 FEISHU_LARK_CLI_PACKAGE 固定版本（如 @larksuite/cli@1.2.3），
        # 避免 @latest 在无人值守部署时引入不可预期的破坏性变更。
        package = os.environ.get("FEISHU_LARK_CLI_PACKAGE", "@larksuite/cli@latest")
        self._run(
            [str(npx), "--yes", package, "install"],
