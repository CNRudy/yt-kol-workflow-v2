...
# Retry policy for transient failures (rate limit / server errors / timeouts).
_RETRY_STATUS = {429, 500, 502, 503, 504}
_MAX_RETRIES = 4
_RETRY_BACKOFF_BASE = 1.5  # seconds, exponential: 1.5, 3, 6, 12
...
class BitableClient:
    ...
    def _request(
        ...
        *,
        ...
        """Perform one HTTP request with unified retry/backoff and JSON guard.

        Retries on 429 / 5xx / network timeouts with exponential backoff, then
        normalises the response so callers never receive a non-JSON body.
        """
        url = self._url(path)
        ...
        for attempt in range(_MAX_RETRIES + 1):
            try:
                ...
                last_exc = exc
                logger.warning("Feishu %s %s 超时 (第 %d 次)", method, path, attempt + 1)
            except requests.RequestException as exc:
                last_exc = exc
                logger.warning("Feishu %s %s 网络错误: %s (第 %d 次)", method, path, exc, attempt + 1)
            else:
                status = resp.status_code
                if status in _RETRY_STATUS and attempt < _MAX_RETRIES:
                    backoff = _RETRY_BACKOFF_BASE * (2 ** attempt)
                    logger.warning(
                        "Feishu %s %s 返回 %d，%.1fs 后重试 (第 %d 次)",
                        method, path, status, backoff, attempt + 1,
                    )
                    time.sleep(backoff)
                    continue
                try:
                    ...
                    # Non-JSON (e.g. HTML error page) — surface a stable dict.
                    return {
                        "code": -1,
                        "msg": f"非JSON响应 (HTTP {status}): {(resp.text or '')[:200]}",
                    }

        # Exhausted retries on a network/timeout error.
        raise RuntimeError(
            f"Feishu {method} {path} 重试 {_MAX_RETRIES} 次后仍失败: {last_exc}"
        )
    ...
    def _put(self, path: str, data: dict) -> dict:
        return self._request("PUT", path, data=data, timeout=60)
