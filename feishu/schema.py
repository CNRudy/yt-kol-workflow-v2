...
def format_influencer_record(channel: dict, rep_video: dict, keyword: str = "") -> dict:
    ...
    fields.update({
        ...
        "邮箱来源": str(channel.get("email_source", "") or "未找到"),
        "邮箱出现视频数": channel.get("email_hit_videos", 0) or 0,
        "候选邮箱": _truncate(str(channel.get("email_candidates", "") or ""), 500),
        "亚马逊推广经验": str(channel.get("amazon_promo_level", "") or "未发现"),
        "Amazon带货视频数": channel.get("amazon_promo_video_count", 0) or 0,
        "推广视频数": channel.get("promo_video_count", 0) or 0,
        "Amazon Storefront": _link(channel.get("amazon_storefront_url", "")),
        "推广证据": _truncate(str(channel.get("promo_evidence", "") or ""), 1000),
        "匹配产品": str(channel.get("match_profile", "") or ""),
        "品牌匹配度": channel.get("brand_fit_score", 0) or 0,
        "内容契合类型": str(channel.get("content_types", "") or ""),
        "匹配关键词": _truncate(str(channel.get("matched_keywords", "") or ""), 200),
        "开发优先级": str(channel.get("dev_priority", "") or "C"),
        "推荐理由": _truncate(str(channel.get("recommend_reason", "") or ""), 500),
...
def format_influencer_video_record(video: dict, channel_title: str = "") -> dict:
    ...
    return {"fields": {
        ...
        "Duration (H:M:S)": str(video.get("duration_hms", "")),
        "Tags": str(video.get("tags", "")),
        "字幕内容": _truncate(video.get("description", ""), 2000),
        "推广类型": str(video.get("promo_label", "") or ""),
        "推广链接": _truncate(str(video.get("promo_links", "") or ""), 1000),
    }}
