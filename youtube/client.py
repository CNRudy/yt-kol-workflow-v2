...
def _build_proxies() -> Dict:
    """Honor explicit HTTPS_PROXY/HTTP_PROXY env vars.

    On macOS, requests falls back to the system proxy (via scutil) and
    ignores env overrides, which can point at a stale Clash port and cause
    ReadTimeout. Passing the env proxy explicitly forces the intended route.
    Returns {} when unset so requests keeps its default behavior.
    """
