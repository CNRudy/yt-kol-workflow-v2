...
def estimate_single_keyword(
    ...
    ``recent_video_count`` must match what Phase D actually fetches per channel
    (workflow/phase_d_detail.py ``run_phase_d`` default is 25).  Keeping both
    driven by the same value prevents the estimate from passing a batch that
    then exhausts quota mid-run.
    ...
    if use_playlist:
        ...
        recent_detail_batches = (estimated_channels * recent_video_count + 49) // 50
...
def print_quota_estimate(
    ...
    for i, (kw, mr) in enumerate(zip(keywords, max_results_list)):
        est = estimate_single_keyword(
            mr, estimated_channels_per_kw, use_playlist, include_detail,
