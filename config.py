...
class FeishuConfig:
    ...
    base_name: str = ""
    hongren_table_id: str = ""  # 派生「红人表」ID；空则用默认，可被 FEISHU_HONGREN_TABLE_ID 覆盖
    proxy: str = ""             # 访问飞书/YouTube 的 HTTPS 代理，如 http://127.0.0.1:7890
...
class WorkflowConfig:
    ...
    recent_video_count: int = 25  # 阶段D每个频道抓取的最近视频数（估算与实际抓取共用此值）
        ...
        with open(path, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                try:
                    ...
                    print(f"[警告] 关键词 '{keyword}' 的 max_results 非法（'{max_results_str}'），"
                          f"已回退为默认 {default_max_results}")
