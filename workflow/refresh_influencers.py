...
# "promo" backfills the Amazon-promotion grading AND re-derives the contact
# email from video descriptions, which is where most creators publish it.
ALL_REFRESH_FIELDS = {"name", "activity", "assessment", "rep-title", "promo"}
...
def refresh_influencers(
    *,
    ...
    if records and selected & {"name", "assessment", "promo"}:
    ...
    for record in records:
        ...
        recent_error = ""
        if selected & {"activity", "assessment", "promo"}:
        ...
        if "promo" in selected:
            promo = aggregate_channel_promo(recent_videos)
            update_fields["亚马逊推广经验"] = promo["amazon_promo_level"]
            update_fields["Amazon带货视频数"] = promo["amazon_promo_video_count"]
            update_fields["推广视频数"] = promo["promo_video_count"]
            update_fields["推广证据"] = promo["promo_evidence"][:1000]
            if promo["amazon_storefront_url"]:
                update_fields["Amazon Storefront"] = promo["amazon_storefront_url"]
            promo_counts[promo["amazon_promo_level"]] = (
                promo_counts.get(promo["amazon_promo_level"], 0) + 1
            )

            # Re-derive the contact email now that we have video descriptions.
            # Never clobber an address the user already has on the row.
            found = extract_contact_email(
                description,
                [v.get("description", "") for v in recent_videos],
            )
            if found["contact_email"] and not field_to_text(existing.get("联系邮箱")):
                update_fields["联系邮箱"] = found["contact_email"]
                update_fields["邮箱状态"] = "已获取"
                update_fields["邮箱来源"] = found["email_source"]
                update_fields["邮箱出现视频数"] = found["email_hit_videos"]
                email_found_count += 1
            if found["email_candidates"]:
                update_fields["候选邮箱"] = ", ".join(found["email_candidates"])[:500]
...
def _estimate_quota(records: Sequence[Dict[str, Any]], selected: Set[str]) -> int:
    ...
    if not count:
        return 0
    cost = 0
    if selected & {"name", "assessment", "promo"}:
